...
module.exports = (function() {
  ...
  SoundCloud.prototype.delete = function(path, params, callback) {
    this.makeCall('DELETE', params, path, callback);
  };
  ...
  SoundCloud.prototype.makeCall = function(method, path, params, callback) {
    if (typeof params === 'function') {
      callback = params;
      params = {};
    } else if (typeof params !== 'object') {
      throw 'Params must be an object, ' + typeof params + ' provided';
    }

    var keys = [];
    var i, tot;
    if (params && typeof params.track === 'object') {
      keys = Object.keys(params.track);

      for (i = 0, tot = keys.length; i < tot; ++i) {
        params['track[' + keys[i] + ']'] = params.track[keys[i]];
      }
  ...
  function request(data, callback) {
    ...
    if (data.accessToken) {
      options.headers.Authorization = 'OAuth ' + data.accessToken;
    }
